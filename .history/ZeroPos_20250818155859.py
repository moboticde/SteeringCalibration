...
# ------------------- CONFIG -------------------
IMAGE_PATH = r'C:\Users\DaniilYegarmin\OneDrive - Mobotic GmbH\Desktop\Photos\P7.jpg'
...
def detect_main_hole(contours, hierarchy):
    """
    Detect the main circular hole in the image.
    ...
    Returns:
        tuple: (main_hole contour, circularity) or (None, 0) if no hole found
    """
    main_hole = None
    max_circularity = 0
...
def detect_holes(contours, hierarchy, center_x, center_y, inner_radius, outer_radius):
    """
    Detect holes in the image that meet specific criteria.
    
    Args:
        contours: List of contours from cv2.findContours
        hierarchy: Contour hierarchy information
        center_x: X coordinate of reference center
        center_y: Y coordinate of reference center
        inner_radius: Minimum radius from center
        outer_radius: Maximum radius from center
        
    Returns:
        list: List of detected hole contours
    """
    holes = []
