...
MIN_AREA = 2
R_MIN, R_MAX = 10, 100
CIRC_MIN = 0.4  # Lowered to accept partial circles
ROUND_RANGE = (0.75, 1.20)  # Adjusted for partial circles
MIN_ARC_PERCENT = 0.6  # Minimum percentage of circle perimeter required
TARGET_RADIUS = 75  # Target circle radius
RADIUS_TOLERANCE = 5  # How much the radius can differ from target
INNER_RADIUS = 330
OUTER_RADIUS = 420

# --- main-hole shape checks (stricter than for other holes) ---
MAIN_CIRC_MIN = 0.50          # 4πA / P²  → 1.0 is a perfect circles
MAIN_ROUND_RANGE = (0.70, 1.20)  # A / (πr²) should be close to 1
MAIN_MIN_ARC_PERCENT = 0.65    # how much of the circumference is present
MAIN_MIN_POINTS = 40           # avoid tiny/noisy contours
# ----------------------------------------------
...
# Find the main circle (must be child of something and pass circularity)
main_hole = None
for i, cnt in enumerate(contours):
    # keep only inner (hole) contours
    ...
    # Circle fit & radii checks
    (_, r) = cv2.minEnclosingCircle(cnt)
    r = float(r)
    if not (abs(r - TARGET_RADIUS) <= RADIUS_TOLERANCE and R_MIN <= r <= R_MAX):
        continue

    # Perimeter & circularity metrics
    peri_closed = cv2.arcLength(cnt, True)
    if peri_closed <= 1e-6:
        continue

    circularity = 4.0 * np.pi * area / (peri_closed * peri_closed)
    roundness = area / (np.pi * r * r + 1e-6)

    # how much of the fitted circle's circumference is covered by the contour
    full_circumference = 2.0 * np.pi * r
    peri_open = cv2.arcLength(cnt, False)
    arc_percentage = min(peri_open, peri_closed) / (full_circumference + 1e-6)

    # accept only truly round-ish contours
    if (circularity >= MAIN_CIRC_MIN and
        MAIN_ROUND_RANGE[0] <= roundness <= MAIN_ROUND_RANGE[1] and
        arc_percentage >= MAIN_MIN_ARC_PERCENT):
        main_hole = cnt
        break
