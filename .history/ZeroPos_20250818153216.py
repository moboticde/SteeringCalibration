...
MIN_ARC_PERCENT = 0.4 # Minimum percentage of circle perimeter required
...
MIN_LINE_LENGTH = 50  # Minimum length for vertical lines
...
def detect_vertical_lines(edges, angle_tolerance=10):
    ...
    if lines is not None:
        for line in lines:
            ...
            # Check if line is vertical (90 degrees ± tolerance)
...
for i, cnt in enumerate(contours):
    ...
    if area < MIN_AREA_MAIN:  # Use larger minimum area
        continue
...
print(f"Number of vertical lines detected: {len(vertical_lines)}")