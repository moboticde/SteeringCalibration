...
MIN_AREA = 2
R_MIN, R_MAX = 2, 100
CIRC_MIN = 0.05  # Lowered to accept partial circles
ROUND_RANGE = (0.3, 1.50)  # Adjusted for partial circles
...
for i, cnt in enumerate(contours):
    ...
    if ((circularity >= CIRC_MIN and ROUND_RANGE[0] <= roundness <= ROUND_RANGE[1]) or 
        (arc_percentage >= MIN_ARC_PERCENT)) and (R_MIN <= r <= R_MAX):
        holes.append(cnt)
