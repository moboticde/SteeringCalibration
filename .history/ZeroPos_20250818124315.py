...
# First pass - find the main circle
holes = []
for i, cnt in enumerate(contours):
    ...
    # Only look for circles matching our target radius
    if abs(r - TARGET_RADIUS) <= RADIUS_TOLERANCE:
        holes.append(cnt)
        break  # Stop after finding the first matching circle

# Align image if we found the main circle
if holes:
    # Get the center of the detected circle
    ...
    if M["m00"] != 0:
        ...
        # Update edges and contours after translation
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        blurred = cv2.GaussianBlur(gray, (5, 5), 0)
...
# Second pass - detect all holes in aligned image
holes = []
for i, cnt in enumerate(contours):
    ...
    # Accept if it's either a full circle or a significant portion of one
    if (((circularity >= CIRC_MIN and ROUND_RANGE[0] <= roundness <= ROUND_RANGE[1]) or 
        ...
        holes.append(cnt)
# Add this function after the center_of() function:
