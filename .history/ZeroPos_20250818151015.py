...
main_hole = None
max_circularity = 0  # Track the most circular contour

for i, cnt in enumerate(contours):
    if hierarchy[0][i][3] != -1:  # Changed to detect parent contours
        continue
    ...
    # Calculate circularity metrics (same as hole detection)
    full_circle_perimeter = 2 * np.pi * r
    peri = cv2.arcLength(cnt, True)  # True for closed contour
    ...
    if (((circularity >= CIRC_MIN and ROUND_RANGE[0] <= roundness <= ROUND_RANGE[1]) or 
        ...
        abs(r - TARGET_RADIUS) <= RADIUS_TOLERANCE):
        # Keep the most circular contour
...
if main_hole is not None:
    ...
    plt.title(f'Main Circle Detection (Circularity: {max_circularity:.2f})')
    plt.show()
