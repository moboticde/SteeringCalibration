...
# ------------------- CONFIG -------------------
IMAGE_PATH = r'C:\Users\DaniilYegarmin\OneDrive - Mobotic GmbH\Desktop\Photos\P5.jpg'
...
# Update the CONFIG section parameters for main circle
MIN_AREA_MAIN = 80000  # Increased minimum area
TARGET_RADIUS_MAIN = 400  # Adjusted target radius
RADIUS_TOLERANCE_MAIN = 70  # Increased tolerance
CIRC_MIN_MAIN = 0.4  # Reduced circularity requirement
...
main_hole = None
max_area = 0  # Track by area instead of circularity
...
for i, cnt in enumerate(contours):
    ...
    # Modified check: prioritize largest area that meets basic criteria
    if (circularity >= CIRC_MIN_MAIN and 
        abs(r - TARGET_RADIUS_MAIN) <= RADIUS_TOLERANCE_MAIN and 
        area > max_area):
        max_area = area
        main_hole = cnt

# Add debug info to show more metrics
if main_hole is not None:
    debug_vis = img.copy()
    cv2.drawContours(debug_vis, [main_hole], -1, (0, 255, 0), 2)
    area = cv2.contourArea(main_hole)
    (_, r) = cv2.minEnclosingCircle(main_hole)
    peri = cv2.arcLength(main_hole, True)
    circularity = 4 * np.pi * area / (peri * peri + 1e-6)
    print(f"Main circle metrics:")
    print(f"Area: {area:.0f}")
    print(f"Radius: {r:.1f}")
    print(f"Circularity: {circularity:.2f}")
    plt.figure(figsize=(12, 8))
    plt.imshow(cv2.cvtColor(debug_vis, cv2.COLOR_BGR2RGB))
    plt.title(f'Main Circle Detection\nArea: {area:.0f}, Radius: {r:.1f}, Circularity: {circularity:.2f}')
    plt.show()
else:
