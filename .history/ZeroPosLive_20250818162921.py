...
FPS_TARGET = 30
INPUT_SCALE = 0.5  # Scale input frames to 50% for processing
DISPLAY_SCALE = 0.75  # Scale output for display
...
def process_frame(frame):
    ...
    # Scale input frame
    frame = rescale_frame(frame, INPUT_SCALE)
    H, W = frame.shape[:2]
    
    # Scale ROI and parameters
    ...
    if main_hole is not None:
        ...
        if center:
            ...
            holes = detect_holes(contours, hierarchy, 
                               center_x=center_x,
                               center_y=center_y,
                               inner_radius=scaled_inner,
                               outer_radius=scaled_outer)
            
            cv2.drawContours(vis, holes, -1, (255, 0, 0), 2)
            
            # Get hole centers
            hole_centers = [c for c in (center_of(h) for h in holes) if c is not None]
            hole_centers_roi = [(x, y) for (x, y) in hole_centers 
                              if (scaled_x1 <= x <= scaled_x2 and scaled_y1 <= y <= scaled_y2)]
            
            # Draw hole centers
            for x, y in hole_centers_roi:
...
def main():
    ...
    # Set camera properties - adjust these for your camera
    ...
    while True:
        ...
        if DISPLAY_SCALE != 1.0:
            result = rescale_frame(result, DISPLAY_SCALE)
