...
def detect_lines(edges, angle_tolerance=10):
    """
    ...
    # Match your existing scaling for the red circles
    ...
    def in_circle(x, y):
        ...
        return dx*dx + dy*dy <= scaled_outer * scaled_outer
    ...
    if raw is not None:
        for x1, y1, x2, y2 in raw.reshape(-1, 4):
            # Keep only segments fully inside the outer circle
            if not (in_circle(x1, y1) and in_circle(x2, y2)):
                continue

            # Angle w.r.t. x-axis -> tilt from vertical
            ang_x = abs(np.degrees(np.arctan2(y2 - y1, x2 - x1)))  # 0..180 vs x-axis
            tilt = abs(90.0 - ang_x)                                # 0° = vertical

            # Vertical & long enough
            if tilt <= angle_tolerance and np.hypot(x2 - x1, y2 - y1) >= MIN_LINE_LENGTH:
                lines_info.append((x1, y1, x2, y2, tilt))
