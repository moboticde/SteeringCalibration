...
MIN_AREA = 2
R_MIN, R_MAX = 2, 100                 # min/max hole radius in pixels (adjust if needed)
CIRC_MIN = 0.2                      # circularity threshold (≈1.0 for perfect circle)
ROUND_RANGE = (0.75, 1.50)           # area/(pi*r^2) tolerance around a circle
# ---------------------------------------------
...
edges = cv2.Canny(eroded, 10, 100)
# Close small gaps in edges
edges = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, kernel)

# Adjust detection parameters
CIRC_MIN = 0.15  # Lower threshold to allow partial circles
ROUND_RANGE = (0.6, 1.6)  # Wider range for less perfect circles
...
for i, cnt in enumerate(contours):
    ...
    if circularity >= CIRC_MIN and ROUND_RANGE[0] <= roundness <= ROUND_RANGE[1] and (R_MIN <= r <= R_MAX):
        holes.append(cnt)
