...
def process_frame(frame):
    ...
    best_tilt = None  # in degrees
    if lines is not None:
        for x1, y1, x2, y2 in lines.reshape(-1, 4):
            # angle w.r.t x-axis, then convert to tilt from vertical
            ang_x = abs(np.degrees(np.arctan2(y2 - y1, x2 - x1)))   # 0..180 wrt x-axis
            tilt = abs(90.0 - ang_x)                                # 0° = perfectly vertical

            if tilt <= VERTICAL_ANGLE_TOLERANCE:
                # draw the line
                cv2.line(vis, (x1, y1), (x2, y2), (0, 255, 150), 2)
                # annotate small tilt near its midpoint
