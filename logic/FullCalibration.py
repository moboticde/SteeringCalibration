...
def calibrate_one_encoder(
    ...
    analyze = None
    try:
        def write_params_preserving_and_report(label: str) -> int:
            ret = _write_params_preserving(handle, protected_params, label)
            if zero_preserving_callback is not None:
                zero_preserving_callback(
                    {
                        "label": label,
        ...
        for attempt_idx in range(1, max_calibration_attempts + 1):
            ...
            if ret != MU_OK or write_params_preserving_and_report(
                ...
                quality_callback(
                    {
                        ...
                        "analog_valid": metrics.analog_valid,
                        "nonius_valid": metrics.nonius_valid,
                        "max_abs_analog_residual_lsb": metrics.max_abs_analog_residual_lsb,
                        "max_abs_phase_margin_pct_of_range_limit": (
                            metrics.max_abs_phase_margin_pct_of_range_limit
                        ),
                        "max_abs_phase_error_lsb": metrics.max_abs_phase_error_lsb,
...
def main(
    ...
    try:
        ...
        ok_enc_1 = calibrate_one_encoder(
            ...
            zero_preserving_callback=(
                lambda data: result_callback(
                    {"event": "zero_preserving", "encoder": 1, **data}
                )
                if result_callback is not None
                else None
            ),
        ...
        ok_enc_2 = calibrate_one_encoder(
            ...
            zero_preserving_callback=(
                lambda data: result_callback(
                    {"event": "zero_preserving", "encoder": 2, **data}
                )
                if result_callback is not None
                else None
            ),
