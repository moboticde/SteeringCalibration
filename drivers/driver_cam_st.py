...
MIN_LINE_LENGTH = 90  # Minimum length of detected lines in pixels
MAX_LINE_LENGTH = 130 # Maximum length of detected lines in pixels
...
def process_image(image):
    ...
    dilated = cv2.dilate(blurred, kernel1, iterations=1) # save this 1
    eroded = cv2.erode(dilated, kernel2, iterations=3) # save this 3
    edges = cv2.Canny(eroded,100, 300) # was 150,300
...
def detect_lines(edges):
    ...
    Returns:
        ...
        best_ang_x: float or None                        # ang_x of the longest linenow
    """
    H, W = edges.shape[:2]
    cx, cy = W // 2 - 6, H // 2 - 85 
    ...
    raw = cv2.HoughLinesP(
        edges, rho=3, theta=np.pi/360, threshold=10,
        minLineLength = MIN_LINE_LENGTH, maxLineGap=8
    )
    ...
    if raw is not None:
        for x1, y1, x2, y2 in raw.reshape(-1, 4):
            ...
            # --- enforce [60, 100] px length window ---
...
def process_frame(frame, visualize=False):
    ...
    # ROI bounds
    def get_roi():
        center_x, center_y = int(W / 2 - 6), int(H / 2 - 85)
    ...
    # Lines inside circle
    lines_info, _ = detect_lines(edges)
