...
def _load_flash_config_zero_with_fake_mu():
    ...
    try:
        ...
        module_path = Path(__file__).resolve().parents[1] / "logic" / "FlashConfigZero.py"
...
class VisualZeroMathTests(unittest.TestCase):
    def test_zero_plan_uses_micontrol_config_and_zero_preset_for_both_encoders(self):
        self.assertEqual(
            [
                (enc_idx, dig_out, config_path, preset)
                for enc_idx, dig_out, config_path, preset in fz.ZERO_PRESET_PLAN
            ],
            [
                (1, True, fz.ZERO_CONFIG_PATH, 0),
