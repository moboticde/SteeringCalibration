...
def _load_full_calibration_with_fake_hardware():
    ...
    try:
        ...
        steering_script = types.ModuleType("FlashSteeringScript")
        steering_script.import_script_via_qr = lambda: None
        steering_script.load_config_function = lambda _path: (None, None, 0)
        sys.modules["FlashSteeringScript"] = steering_script

        config_processing = types.ModuleType("config_processing")
        config_processing.nmt_reset_node_compat = lambda _nmt: None
        sys.modules["config_processing"] = config_processing

        interrupt_guard = types.ModuleType("interrupt_guard")
        interrupt_guard.install_deferred_keyboard_interrupt = lambda **_kwargs: (lambda: None)
        sys.modules["interrupt_guard"] = interrupt_guard

        module_path = Path(__file__).resolve().with_name("FullCalibration.py")
