...
from calibration_quality import build_quality_report, evaluate_quality
...
def check_encoder_quality(
    ...
    try:
        ...
        if ret != 0:
            print(f"[ERROR] MU_ReadParams failed rc={ret} ({_mu_last_error(handle)})")
            return False
        ...
        print(f"[RESULT] encoder_{enc_idx + 1} quality={'PASS' if evaluation.ok else 'FAIL'}")
        print(
        ...
        print(
            f"[RESULT] encoder_{enc_idx + 1} "
            f"phase_clearance_upper/lower="
            f"{metrics.upper_phase_margin_pct:.3f}%/{metrics.lower_phase_margin_pct:.3f}%"
        )
        print(f"[RESULT] encoder_{enc_idx + 1} reason={evaluation.reason}")
...
def main(
    ...
    from FullCalibration import restart_controller
